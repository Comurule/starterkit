...
exports.createPreference = async (req, res) => {
    ...
    try {
        ...
        //generate a Code
        let pcCode = codeGen('p', 3);
        await checkCodeGen('p',3, PreferenceCenter,{pcCode}, pcCode);
...
exports.updatePreference = async (req, res) => {
    try {
        ...
        if(checkPreference && checkPreference.id != req.params.preferenceId) {
            ...
            //Success response
            successResWithData( res, 'Preference updated successfully.', data );
        }
...
const validateInputs = async (req, res) => {
    const { name, tier, parentPC, displayType } = req.body;
    
    //To check against empty fields
    if(!name || !tier || !displayType || name == '' || tier == '' || displayType == '') 
