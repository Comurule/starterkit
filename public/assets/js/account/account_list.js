...
const fetchData = async () =>{
  try {
    const contacts = await fetch('/api/v1/accounts');
  ...
  var datatable = $('.kt_datatable').KTDatatable({
    ...
    columns: [
      ...
      },
      {
        ...
        template: function (row) {
          return `
            <a href="/main/accounts/${row.id}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="View Account Details">
              <i class="la la-eye"></i>
            </a>
            <a href="/main/accounts/${row.id}/update" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit Account Details">
